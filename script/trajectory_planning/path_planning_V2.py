...
z_offset = 100
# start and end point
P0 = np.array([-2000, 0, -280])
P3 = np.array([2000, 0, -280])
# via points
...
def time_taken_interval(s, delta_s):
    global state, s_1, current_vel, delta_t
    ...
    if state == STATE_ACC:      
        ...
        # print(f"delta_t={delta_t}\tcurrent_ vel={current_vel}")
        # v1 = v0 + a0*t
        current_vel = current_vel + max_acc * delta_t

        if s >= 0.5:
            state = STATE_DEC
        elif current_vel > max_vel:            #get_velocity_bezier_curve(s, delta_s, delta_t)
            state = STATE_VEL
            s_1 = s
